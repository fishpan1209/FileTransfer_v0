...
public class Transfer6 {
				...
				for (FileStatus file : fileStatus) {
					...
					if (hdfs.isDirectory(file.getPath())) {
						...
						*/
						...
						header.append("fileName: "+file.getPath().getName());
						long fileSize = file.getLen();
						long creationTime = file.getModificationTime();
						header.append(" fileSize: "+fileSize);
						header.append(" file creationTime: "+creationTime);
						header.append("\n");
						sendFile(file.getPath(), destDir,hdfs, header.toString());
					}
				}
			}
		}
	...
	public void sendFile(Path hdfsSrc, Path hdfsDst, FileSystem hdfs, String header) throws IOException {
		...
		try {
			FSDataInputStream in = hdfs.open(hdfsSrc);
			FSDataOutputStream out = hdfs.create(hdfsDst);
	...
	public static void main(String[] args) throws Exception {
		...
		t6.transfer(hdfs, hdfsSrc, hdfsDst);
